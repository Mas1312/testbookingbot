...
#   true (продакшен/облако, например Render) — Telegram сам присылает обновления
#     на POST {WEBAPP_URL}/webhook, всё работает внутри одного процесса server.py,
#     отдельный bot.py запускать не нужно и нельзя (конфликт с вебхуком).
